...
class DeveloperRanking:
    ...
    def calculate_time_spent(self, developer):
        ...
        bug_time = dev_issues[dev_issues['fields.issuetype.name'] == 'Bug']['fields.timespent'].sum()
        subtask_time = dev_issues[dev_issues['fields.issuetype.name'] == 'Sub-task']['fields.timespent'].sum()
        return bug_time, subtask_time
    ...
    def count_bugs_by_criticality(self, developer):
        ...
        normal = dev_bugs[dev_bugs['fields.priority.name'] == 'Normal'].shape[0]
        critical = dev_bugs[dev_bugs['fields.priority.name'] == 'Critical'].shape[0]
        blocker = dev_bugs[dev_bugs['fields.priority.name'] == 'Blocker'].shape[0]
    ...
    def calculate_avg_completion_time(self, developer):
        ...
        dev_issues = dev_issues[(dev_issues['completion_time'] >= 0) & (dev_issues['completion_time'].notna())]
        return dev_issues['completion_time'].mean()
    ...
    def calculate_days_logged_8_hours(self, developer):
        ...
        total_time_spent = dev_issues['fields.timespent'].sum()
        days_logged_8_hours = total_time_spent / (8 * 3600)  # 8 hours in seconds
        return round(days_logged_8_hours)
    ...
    def calculate_score(self, row):
        score = (
            ...
            (2 - min(abs(1 - row['EstimationAccuracy']), 1)) * 50 +
